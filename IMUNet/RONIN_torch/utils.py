...
class ProposedSequence(CompiledSequence):
    ...
    def __init__(self, data_path, **kwargs):
        ...
        self.ts, self.features, self.targets, self.orientations, self.gt_pos = None, None, None, None, None
        self.w = kwargs.get('interval', 1)
        self.ori_source = kwargs.get('ori_source', 'rv')
        self.use_init_rotor = kwargs.get('use_init_rotor', True)
        self.info = {}
    ...
    def load(self, path):
        ...
        self.info['path'] = osp.split(path)[-1]
        self.info['ori_source'] = self.ori_source
        print(path)
        if osp.exists(osp.join(path, 'processed/data.csv')):
            imu_all = pandas.read_csv(osp.join(path, 'processed/data.csv'))
        else:
        ...
        if self.ori_source == 'game_rv':
            rv_cols = ['game_rv_w', 'game_rv_x', 'game_rv_y', 'game_rv_z']
        else:
            rv_cols = ['rv_w', 'rv_x', 'rv_y', 'rv_z']
        rv = quaternion.from_float_array(imu_all[rv_cols].values)

        if self.use_init_rotor:
            # Align the rotation vector to the ARCore groundtruth orientation at frame 0.
            # Not available at real-time inference (no groundtruth), so this is optional.
            init_tango_ori = quaternion.quaternion(*imu_all[['ori_w', 'ori_x', 'ori_y', 'ori_z']].values[0])
            init_rotor = init_tango_ori * rv[0].conj()
            ori = init_rotor * rv
        else:
            ori = rv
        ...
        self.targets = (tango_pos[self.w:, :2] - tango_pos[:-self.w, :2]) / (ts[self.w:] - ts[:-self.w])
        self.gt_pos = tango_pos
        self.orientations = quaternion.as_float_array(rv)
        print(self.ts.shape, self.features.shape, self.targets.shape, self.gt_pos.shape, self.orientations.shape,
              self.w)
...
def load_cached_sequences(seq_type, root_dir, data_list, cache_path, **kwargs):
    ...
    ori_source = kwargs.get('ori_source', 'rv')
    use_init_rotor = kwargs.get('use_init_rotor', True)
    ...
    if cache_path is not None and cache_path not in ['none', 'invalid', 'None']:
        ...
        if osp.exists(osp.join(cache_path, 'config.json')):
            ...
            if info.get('grv_only', 'False') != str(grv_only):
                ...
                cache_path = 'invalid'
            if info.get('ori_source', 'rv') != ori_source:
                warnings.warn('The cached dataset has different flag in "ori_source". Ignore')
                cache_path = 'invalid'
            if info.get('use_init_rotor', 'True') != str(use_init_rotor):
                warnings.warn('The cached dataset has different flag in "use_init_rotor". Ignore')
                cache_path = 'invalid'
        else:
            info = {'feature_dim': seq_type.feature_dim, 'target_dim': seq_type.target_dim,
                    'aux_dim': seq_type.aux_dim, 'grv_only': str(grv_only),
                    'ori_source': ori_source, 'use_init_rotor': str(use_init_rotor)}
